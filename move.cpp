...
void do_combat(dungeon *d, character *atk, character *def)
{
  ...
  if (def->alive) {
    if (atk == d->PC)
    {
      ...
      {
        if(d->PC->equip[i] != NULL)
        {
          damage_dealt += d->PC->equip[i]->roll_dice();
	       }
      } damage_dealt += atk->damage->roll();

      if(atk == d->PC){
	       if(damage_dealt > def->hp)
         {
	          character_die(def);
	          d->num_monsters--;
	          io_queue_message("%s attacks %s", atk->name, character_get_name(def));
          ...
          {
	           def->hp -= damage_dealt;
	            io_queue_message("You attack %s dealing %u damage", character_get_name(def), damage_dealt);
	        }
        }
    }
    ...
    if(def == d->PC){
      ...
      {
        def->hp -= damage_dealt;
        io_queue_message("%s is dealt %u damage", def->name, damage_dealt);
      }
    }
  }
}
