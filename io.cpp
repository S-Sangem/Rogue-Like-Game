...
void io_equipment(dungeon *d)
{
  ...
  if(d->PC->equip.size() > 0)
  {
    ...
    {
      name = d->PC->equip[i]->name;
      type = d->PC->equip[i]->type;
      str = object_class[type];

      mvprintw(i+4, 64, " %s ", str.c_str());
...
void io_inventory(dungeon *d)
{
  ...
  if(d->PC->carry.size() > 0)
  {
    ...
    for(i=0; i < d->PC->carry.size(); i++)
    {
      name = d->PC->carry[i]->name;
      type = d->PC->carry[i]->type;
      str = object_class[type];
